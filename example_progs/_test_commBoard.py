...
def test1(funct, ser_ports, msg_queue):
	...
	for ser in ser_ports:
		...
		ser.TX_thread = txThread(
			'Serial TX {port} @ {baud}'.format(port=ser.port, baud=ser.baudrate),
		...
		ser.RX_thread = rxThread(
			'Serial RX {port} @ {baud}'.format(port=ser.port, baud=ser.baudrate),
	...
	msg_queue.put((None, None, 'Sleep: {time:<6.4} sec.'.format(time=TEST_BREAK_TIME)))
	print_msg_queue(msg_queue)
