...
class PacketGenerator(threadMonitor.ThreadMonitor):
	...
	def __init__(self, max_queue_size=INITIAL_PACKET_NUMBER, num_rand_bytes=RAND_DATA_SIZE,
		...
		:param num_rand_bytes: number of random bytes to generate (Larger is better. Only one time when assembling data)
		...
		if num_rand_bytes > (
				(len(struct.pack('!H', 0)) * 2) + MAX_PACKET_DATA_LENGTH +
		...
		if printable_chars:
			self._random_data = ''.join([chr(self._rand.randint(33, 126)) for _ in xrange(num_rand_bytes)])
		else:
			self._random_data = ''.join([chr(self._rand.randint(1, 255)) for _ in xrange(num_rand_bytes)])
		...
		if self.msg_queue:
			self.msg_queue.put((None, {'num_rand_bytes': num_rand_bytes}, GENERATE_DATA))

	def change_packet_length(self, packet_length=PACKET_SIZE):
		"""
		If the packet length is set to None then a random length is used
	...
	def locked_running(self):
		try:
			...
			try:
				# loop until Queue.Empty exception is thrown
				while True:
	...
	def make_packet(self):
		...
		while len(packet_data) < data_len:
			...
			if self._start_index > len(self._random_data):
				self._start_index = 0
		# get the next packet_id
		PacketGenerator.allocated_lock.acquire()
		packet_id = self._rand.choice(list(PacketGenerator.ALLOCATABLE_PACKET_ID - PacketGenerator.allocated))
		# Set packetID as used
		PacketGenerator.allocated.add(packet_id)
		PacketGenerator.allocated_lock.release()
		# assemble the packet
		packet = struct.pack('!H', packet_id) + struct.pack('!H', local_packet_length) + packet_data + data_hash.digest()
		# place the information into the Queue
		return packet, packet_id, local_packet_length, data_hash.hexdigest(),


if __name__ == '__main__':
	import tests.testPacketGenerator
	tests.testPacketGenerator.runtests()
